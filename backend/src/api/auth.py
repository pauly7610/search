...
async def register_user(user_data: UserCreate):
    ...
    try:
        ...
        access_token = auth_middleware.create_access_token(
            data={"user_id": user_id, "email": user_data.email, "name": user_data.name}
        )
...
async def initiate_app_connection(
    app_name: str, current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        auth_data = await agent_auth_manager.get_auth_url(
            app_name, current_user["user_id"]
        )
...
async def oauth_callback(
    ...
    try:
        ...
        if connection_status["status"] == "connected":
            logger.info(
                f"Connection completed for {current_user['email']} -> {connection_status['app']}"
            )
            return RedirectResponse(
            ...
            logger.warning(
                f"Connection failed for {current_user['email']}: {connection_status}"
            )
            return RedirectResponse(
        ...
        return RedirectResponse(
            url="/dashboard?error=callback_failed", status_code=status.HTTP_302_FOUND
        )
...
async def get_user_connections(
    ...
    """
    try:
        connections = await agent_auth_manager.get_user_connections(
...
async def execute_agent_action(
    ...
    try:
        ...
        app_connected = any(
            conn["app"] == app_name and conn["status"] == "connected"
        ...
        logger.info(
            f"Action executed: {action} on {app_name} for {current_user['email']}"
        )
