...
def get_knowledge_base(q: str = Query(None, description="Search query"), top_k: int = 5, filters: str = Query(None, description="JSON dict of metadata filters"), ranking_boosts: str = Query(None, description="JSON dict of ranking boosts")):
    if not q:
    ...
    results = semantic_kb.hybrid_search(q, top_k=top_k, filters=filters_dict, ranking_boosts=ranking_boosts_dict)
