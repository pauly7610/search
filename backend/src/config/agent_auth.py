...
class AgentAuthSettings(BaseSettings):
    ...
    composio_api_url: str = Field(default="https://api.composio.dev/v1/", description="Composio API URL")
    
    # Authentication Configuration
    agent_auth_enabled: bool = Field(default=True, description="Enable AgentAuth features")
    oauth2_redirect_uri: str = Field(default="http://localhost:3000/auth/callback", description="OAuth2 redirect URI")
...
class AgentAuthManager:
    ...
    def __init__(self):
        if agent_auth_settings.composio_api_key:
            ...
            self.toolset = None
            logger.warning("Composio API key not provided. AgentAuth features will be limited.")
    ...
    async def get_auth_url(self, app_name: str, user_id: str) -> Dict[str, Any]:
        ...
        """
        if not self.toolset:
            raise Exception("Composio API key not configured. Please set COMPOSIO_API_KEY environment variable.")
    ...
    async def verify_connection(self, connection_id: str, user_id: str) -> Dict[str, Any]:
        """
    ...
    async def get_user_connections(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        """
        if not self.toolset:
            logger.warning("Composio API key not configured. Returning empty connections list.")
            return []
