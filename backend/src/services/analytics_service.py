...
class AnalyticsService:
    ...
    def get_response_time_trend(self, days: int = 30) -> List[Dict]:
        ...
        return [
            {
                "date": date,
                "responseTime": sum(times) / len(times) if times else 0
            }
    ...
    def get_satisfaction_trend(self, days: int = 30) -> List[Dict]:
        ...
        return [
            {
                "date": date,
                "satisfaction": sum(scores) / len(scores) if scores else 0
            }
    ...
    def get_overview(self) -> Dict:
        return {
            ...
            "averageResponseTime": f"{sum(rt['response_time'] for rt in self.response_times) / len(self.response_times):.1f}s" if self.response_times else "0s",
            "satisfactionRate": f"{sum(score['score'] for score in self.satisfaction_scores) / len(self.satisfaction_scores):.0f}%" if self.satisfaction_scores else "0%",
